...
import           Servant                       ((:<|>) ((:<|>)), Application, Handler (Handler), ServantErr (..),
                                                err400, hoistServer, serve)
import           Servant.Client                (BaseUrl (baseUrlPort), ClientEnv, ServantError, mkClientEnv)
import           Servant.Extra                 (capture)

type AppEffects m = '[NodeFollowerEffect, State MockWalletState, Log, Reader ClientEnv, Error ServantError, Error ServantErr, m]
...
runAppEffects ::
    ...
    -> MockWalletState
    -> Eff (AppEffects m) a
    -> m (Either ServantErr (a, MockWalletState))
